...
import 'package:incometax/itax_ecr_preview.dart';
...
class _ItaxPageState extends State<ItaxPage>
  ...
  Future<void> fetchmainpgData(String biometricId) async {
    try {
      DatabaseEvent event =
          await bioRef.child('maindata').child(biometricId).once();
      DataSnapshot snapshot = event.snapshot;
  ...
  Future<Map<String, Map<String?, dynamic>>> fetchAllMonthData(
      String biometricId) async {
    List<String> months = [
      'jan',
      'feb',
      'mar',
  ...
  Future<Map<String?, dynamic>> fetchMonthData(
      String month, String biometricId) async {
    try {
  ...
  Future<void> fetcharrearData(String biometricId) async {
    try {
      DatabaseEvent event =
          await bioRef.child('arrdata').child(biometricId).once();
      DataSnapshot snapshot = event.snapshot;
  ...
  Future<void> fetchdedData(String biometricId) async {
    try {
      DatabaseEvent event =
          await bioRef.child('deddata').child(biometricId).once();
      DataSnapshot snapshot = event.snapshot;
  ...
  Future<void> fetchitaxData(String biometricId) async {
    try {
      DatabaseEvent event =
          await bioRef.child('itfdata').child(biometricId).once();
      DataSnapshot snapshot = event.snapshot;
  ...
  Future<void> updateItaxold(
      String? biometricId, int varNitpi, int varEp) async {
    if (biometricId == null) {
      if (mounted) {
        showDialog(
          context: context,
    ...
    try {
      await bioRef
          .child('itaxold')
          .child(biometricId)
          .set({'biometricid': biometricId, 'nitpi': varNitpi, 'ep': varEp});
    } catch (error) {
      if (mounted) {
        showDialog(
          context: context,
  ...
  Widget _buildIDInput() {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon:
            const Icon(Icons.fingerprint_rounded, color: Color(0xFF3B82F6)),
        contentPadding:
    ...
    return Wrap(
      ...
      children: [
        _buildPrimaryBtn("Export Single", Icons.description_rounded, () {
          if (!_hasSelectedBiometricId()) {
            return;
          }
          createExcel();
        }),
        _buildSecondaryBtn("View ECR", Icons.grid_view_rounded, _previewEcr),
        _buildSecondaryBtn(
            "Export All", Icons.folder_zip_rounded, () => exportAll(false)),
        _buildSecondaryBtn("Export All (PAN)", Icons.credit_card_rounded,
            () => exportAll(true)),
      ],
    );
  }

  bool _hasSelectedBiometricId() {
    if (biometricIdController.text.trim().isNotEmpty) {
      return true;
    }
    showDialog(
      context: context,
  ...
  Future<void> _previewEcr() async {
    if (!_hasSelectedBiometricId()) {
      return;
    }

    final selectedBiometricId = biometricIdController.text.trim();
    await createExcel(biometricId: selectedBiometricId, downloadFile: false);
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => EcrPreviewPage(biometricId: selectedBiometricId),
      ),
    );
  }
  ...
  Future<void> createExcel({
    ...
    biometricId = (biometricId == null || biometricId.isEmpty)
    ...
    final xls.Style commontextStyleBold =
        workbook.styles.add('commontextStyleBold');
    ...
    final xls.Style tableheadingStyle =
        workbook.styles.add('tableheadingStyle');
    ...
    final xls.Style speciallastrowStyle =
        workbook.styles.add('speciallastrowStyle');
    ...
    final xls.Style specialtotalrowStyle =
        workbook.styles.add('specialtotalrowStyle');
    ...
    Map<String, Map<String?, dynamic>> monthdata =
        await fetchAllMonthData(biometricId);
    ...
    await _itaxformPage(
        ...
        formlastrowStyle,
        specialcolStyle,
        specialtotalrowStyle,
        speciallastrowStyle);
    await fetchitaxData(biometricId);
    await _computationPage(biometricId, computationSheet, otherheaderStyle,
        commontextStyle, commontextStyleBold);
    await _itaxOldPage(biometricId, itaxoldSheet, otherheaderStyle,
        commontextStyle, commontextStyleBold);
    await _itaxNewPage(biometricId, itaxnewSheet, otherheaderStyle,
        commontextStyle, commontextStyleBold);
    await _itaxformPage(
        biometricId,
        ecrSheet,
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (_) => AlertDialog(
            title: Text("Error"),
            content: Text(
                'Failed to convert Excel to PDF. Code: ${response.statusCode}'),
            actions: [
              TextButton(
  ...
  Future<void> _itaxformPage(
    ...
    try {
      ...
      int varSplpaymar =
          int.tryParse(marData['splpay']?.toString() ?? '0') ?? 0;
      int varConvmar = int.tryParse(marData['conv']?.toString() ?? '0') ?? 0;
      int varPgmar = int.tryParse(marData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualmar =
          int.tryParse(marData['annual']?.toString() ?? '0') ?? 0;
      int varUniformmar =
          int.tryParse(marData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingmar =
          int.tryParse(marData['nursing']?.toString() ?? '0') ?? 0;
      int varTamar = int.tryParse(marData['ta']?.toString() ?? '0') ?? 0;
      int varDaontamar =
          int.tryParse(marData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalmar =
          int.tryParse(marData['medical']?.toString() ?? '0') ?? 0;
      int varDirtmar = int.tryParse(marData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingmar =
          int.tryParse(marData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((marData['nps'] is int
                  ? marData['nps']
                  : int.tryParse(marData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempmar = ([
      ...
      int varDaext1mar =
          int.tryParse(marData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2mar =
          int.tryParse(marData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3mar =
          int.tryParse(marData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4mar =
          int.tryParse(marData['daext4']?.toString() ?? '0') ?? 0;
      int varSalarymar = [
        varBpmar,
        varDamar,
        varHramar,
      ...
      int varIncometaxmar =
          int.tryParse(marData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietymar =
          int.tryParse(marData['society']?.toString() ?? '0') ?? 0;
      int varRecoverymar =
          int.tryParse(marData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedmar = int.tryParse(marData['med']?.toString() ?? '0') ?? 0;
      int varWatermar = int.tryParse(marData['water']?.toString() ?? '0') ?? 0;
      int varOther2mar = int.tryParse(marData['other']?.toString() ?? '0') ?? 0;
      int varDdext1mar =
          int.tryParse(marData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2mar =
          int.tryParse(marData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3mar =
          int.tryParse(marData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4mar =
          int.tryParse(marData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedmar =
          int.tryParse(marData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalarymar =
          int.tryParse(marData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayapr =
          int.tryParse(aprData['splpay']?.toString() ?? '0') ?? 0;
      int varConvapr = int.tryParse(aprData['conv']?.toString() ?? '0') ?? 0;
      int varPgapr = int.tryParse(aprData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualapr =
          int.tryParse(aprData['annual']?.toString() ?? '0') ?? 0;
      int varUniformapr =
          int.tryParse(aprData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingapr =
          int.tryParse(aprData['nursing']?.toString() ?? '0') ?? 0;
      int varTaapr = int.tryParse(aprData['ta']?.toString() ?? '0') ?? 0;
      int varDaontaapr =
          int.tryParse(aprData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalapr =
          int.tryParse(aprData['medical']?.toString() ?? '0') ?? 0;
      int varDirtapr = int.tryParse(aprData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingapr =
          int.tryParse(aprData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((aprData['nps'] is int
                  ? aprData['nps']
                  : int.tryParse(aprData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempapr = (([
      ...
      int varDaext1apr =
          int.tryParse(aprData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2apr =
          int.tryParse(aprData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3apr =
          int.tryParse(aprData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4apr =
          int.tryParse(aprData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryapr = [
        varBpapr,
        varDaapr,
        varHraapr,
      ...
      int varIncometaxapr =
          int.tryParse(aprData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyapr =
          int.tryParse(aprData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryapr =
          int.tryParse(aprData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedapr = int.tryParse(aprData['med']?.toString() ?? '0') ?? 0;
      int varWaterapr = int.tryParse(aprData['water']?.toString() ?? '0') ?? 0;
      int varOther2apr = int.tryParse(aprData['other']?.toString() ?? '0') ?? 0;
      int varDdext1apr =
          int.tryParse(aprData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2apr =
          int.tryParse(aprData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3apr =
          int.tryParse(aprData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4apr =
          int.tryParse(aprData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedapr =
          int.tryParse(aprData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryapr =
          int.tryParse(aprData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpaymay =
          int.tryParse(mayData['splpay']?.toString() ?? '0') ?? 0;
      int varConvmay = int.tryParse(mayData['conv']?.toString() ?? '0') ?? 0;
      int varPgmay = int.tryParse(mayData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualmay =
          int.tryParse(mayData['annual']?.toString() ?? '0') ?? 0;
      int varUniformmay =
          int.tryParse(mayData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingmay =
          int.tryParse(mayData['nursing']?.toString() ?? '0') ?? 0;
      int varTamay = int.tryParse(mayData['ta']?.toString() ?? '0') ?? 0;
      int varDaontamay =
          int.tryParse(mayData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalmay =
          int.tryParse(mayData['medical']?.toString() ?? '0') ?? 0;
      int varDirtmay = int.tryParse(mayData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingmay =
          int.tryParse(mayData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((mayData['nps'] is int
                  ? mayData['nps']
                  : int.tryParse(mayData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempmay = (([
      ...
      int varDaext1may =
          int.tryParse(mayData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2may =
          int.tryParse(mayData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3may =
          int.tryParse(mayData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4may =
          int.tryParse(mayData['daext4']?.toString() ?? '0') ?? 0;
      int varSalarymay = [
        varBpmay,
        varDamay,
        varHramay,
      ...
      int varIncometaxmay =
          int.tryParse(mayData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietymay =
          int.tryParse(mayData['society']?.toString() ?? '0') ?? 0;
      int varRecoverymay =
          int.tryParse(mayData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedmay = int.tryParse(mayData['med']?.toString() ?? '0') ?? 0;
      int varWatermay = int.tryParse(mayData['water']?.toString() ?? '0') ?? 0;
      int varOther2may = int.tryParse(mayData['other']?.toString() ?? '0') ?? 0;
      int varDdext1may =
          int.tryParse(mayData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2may =
          int.tryParse(mayData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3may =
          int.tryParse(mayData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4may =
          int.tryParse(mayData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedmay =
          int.tryParse(mayData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalarymay =
          int.tryParse(mayData['netsalary']?.toString() ?? '0') ?? 0;

      // ============================= arrearqtr1 ==============================
      int varBparrearqtr1 = int.tryParse('0') ?? 0;
      int varDaarrearqtr1 =
          int.tryParse(arrearData['mmda']?.toString() ?? '0') ?? 0;
      int varHraarrearqtr1 =
          int.tryParse(arrearData['mmhra']?.toString() ?? '0') ?? 0;
      int varNpaarrearqtr1 =
          int.tryParse(arrearData['mmnpa']?.toString() ?? '0') ?? 0;
      ...
      int varDaontaarrearqtr1 =
          int.tryParse(arrearData['mmdaonta']?.toString() ?? '0') ?? 0;
      ...
      int varPcaarrearqtr1 =
          int.tryParse(arrearData['mmpca']?.toString() ?? '0') ?? 0;
      int varNpsemparrearqtr1 = int.tryParse('0') ?? 0;
      int varOtherarrearqtr1 =
          int.tryParse(arrearData['mmother']?.toString() ?? '0') ?? 0;
      ...
      int varSalaryarrearqtr1 = [
        ...
        varPgarrearqtr1,
        varAnnualarrearqtr1,
        varUniformarrearqtr1,
        varNursingarrearqtr1,
        varTaarrearqtr1,
        varDaontaarrearqtr1,
        varMedicalarrearqtr1,
        varDirtarrearqtr1,
        varWashingarrearqtr1,
        varTbarrearqtr1,
        varNightarrearqtr1,
        varDrivearrearqtr1,
        varCyclearrearqtr1,
      ...
      int varIncometaxarrearqtr1 =
          int.tryParse(arrearData['mmext1']?.toString() ?? '0') ?? 0;
      ...
      int varTotaldedarrearqtr1 = [
        varIncometaxarrearqtr1,
        varGisarrearqtr1,
        varGpfarrearqtr1,
        varNpsarrearqtr1,
        varSlfarrearqtr1,
        varSocietyarrearqtr1,
        varRecoveryarrearqtr1,
        varWfarrearqtr1,
        varEpfarrearqtr1,
        varEsiarrearqtr1,
        varMedarrearqtr1,
      ...
      int varNetsalaryarrearqtr1 = varSalaryarrearqtr1 - varTotaldedarrearqtr1;

      // =============================== qtr1 ===============================
      int varBpqtr1 = [varBpmar, varBpapr, varBpmay, varBparrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varDaqtr1 = [varDamar, varDaapr, varDamay, varDaarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varHraqtr1 = [varHramar, varHraapr, varHramay, varHraarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varNpaqtr1 = [varNpamar, varNpaapr, varNpamay, varNpaarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varSplpayqtr1 = [
        varSplpaymar,
        varSplpayapr,
      ...
      int varConvqtr1 = [varConvmar, varConvapr, varConvmay, varConvarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varPgqtr1 = [varPgmar, varPgapr, varPgmay, varPgarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varAnnualqtr1 = [
        varAnnualmar,
        varAnnualapr,
      ...
      int varTaqtr1 = [varTamar, varTaapr, varTamay, varTaarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varDaontaqtr1 = [
        varDaontamar,
        varDaontaapr,
      ...
      int varDirtqtr1 = [varDirtmar, varDirtapr, varDirtmay, varDirtarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varWashingqtr1 = [
        varWashingmar,
        varWashingapr,
      ...
      int varTbqtr1 = [varTbmar, varTbapr, varTbmay, varTbarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varNightqtr1 = [
        varNightmar,
        varNightapr,
      ...
      int varPcaqtr1 = [varPcamar, varPcaapr, varPcamay, varPcaarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varNpsempqtr1 = [
        varNpsempmar,
        varNpsempapr,
      ...
      int varIncometaxqtr1 = [
        varIncometaxmar,
        varIncometaxapr,
        varIncometaxmay,
        varIncometaxarrearqtr1
      ].fold(0, (sum, value) => sum + value);
      int varGisqtr1 = [varGismar, varGisapr, varGismay, varGisarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varGpfqtr1 = [varGpfmar, varGpfapr, varGpfmay, varGpfarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varNpsqtr1 = [varNpsmar, varNpsapr, varNpsmay, varNpsarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varSlfqtr1 = [varSlfmar, varSlfapr, varSlfmay, varSlfarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varSocietyqtr1 = [
        varSocietymar,
        varSocietyapr,
      ...
      int varWfqtr1 = [varWfmar, varWfapr, varWfmay, varWfarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varEpfqtr1 = [varEpfmar, varEpfapr, varEpfmay, varEpfarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varEsiqtr1 = [varEsimar, varEsiapr, varEsimay, varEsiarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varMedqtr1 = [varMedmar, varMedapr, varMedmay, varMedarrearqtr1]
          .fold(0, (sum, value) => sum + value);
      int varWaterqtr1 = [
        varWatermar,
        varWaterapr,
      ...
      int varTotaldedqtr1 = [
        ...
        varTotaldedapr,
        varTotaldedmay,
        varTotaldedarrearqtr1
      ].fold(0, (sum, value) => sum + value);
      int varNetsalaryqtr1 = [
        varNetsalarymar,
        varNetsalaryapr,
        varNetsalarymay,
        varNetsalaryarrearqtr1
      ].fold(0, (sum, value) => sum + value);
      ...
      int varSplpayjun =
          int.tryParse(junData['splpay']?.toString() ?? '0') ?? 0;
      int varConvjun = int.tryParse(junData['conv']?.toString() ?? '0') ?? 0;
      int varPgjun = int.tryParse(junData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualjun =
          int.tryParse(junData['annual']?.toString() ?? '0') ?? 0;
      int varUniformjun =
          int.tryParse(junData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingjun =
          int.tryParse(junData['nursing']?.toString() ?? '0') ?? 0;
      int varTajun = int.tryParse(junData['ta']?.toString() ?? '0') ?? 0;
      int varDaontajun =
          int.tryParse(junData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicaljun =
          int.tryParse(junData['medical']?.toString() ?? '0') ?? 0;
      int varDirtjun = int.tryParse(junData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingjun =
          int.tryParse(junData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((junData['nps'] is int
                  ? junData['nps']
                  : int.tryParse(junData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempjun = (([
      ...
      int varDaext1jun =
          int.tryParse(junData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2jun =
          int.tryParse(junData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3jun =
          int.tryParse(junData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4jun =
          int.tryParse(junData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryjun = [
        varBpjun,
        varDajun,
        varHrajun,
      ...
      int varIncometaxjun =
          int.tryParse(junData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyjun =
          int.tryParse(junData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryjun =
          int.tryParse(junData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedjun = int.tryParse(junData['med']?.toString() ?? '0') ?? 0;
      int varWaterjun = int.tryParse(junData['water']?.toString() ?? '0') ?? 0;
      int varOther2jun = int.tryParse(junData['other']?.toString() ?? '0') ?? 0;
      int varDdext1jun =
          int.tryParse(junData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2jun =
          int.tryParse(junData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3jun =
          int.tryParse(junData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4jun =
          int.tryParse(junData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedjun =
          int.tryParse(junData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryjun =
          int.tryParse(junData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayjul =
          int.tryParse(julData['splpay']?.toString() ?? '0') ?? 0;
      int varConvjul = int.tryParse(julData['conv']?.toString() ?? '0') ?? 0;
      int varPgjul = int.tryParse(julData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualjul =
          int.tryParse(julData['annual']?.toString() ?? '0') ?? 0;
      int varUniformjul =
          int.tryParse(julData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingjul =
          int.tryParse(julData['nursing']?.toString() ?? '0') ?? 0;
      int varTajul = int.tryParse(julData['ta']?.toString() ?? '0') ?? 0;
      int varDaontajul =
          int.tryParse(julData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicaljul =
          int.tryParse(julData['medical']?.toString() ?? '0') ?? 0;
      int varDirtjul = int.tryParse(julData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingjul =
          int.tryParse(julData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((julData['nps'] is int
                  ? julData['nps']
                  : int.tryParse(julData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempjul = (([
      ...
      int varDaext1jul =
          int.tryParse(julData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2jul =
          int.tryParse(julData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3jul =
          int.tryParse(julData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4jul =
          int.tryParse(julData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryjul = [
        varBpjul,
        varDajul,
        varHrajul,
      ...
      int varIncometaxjul =
          int.tryParse(julData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyjul =
          int.tryParse(julData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryjul =
          int.tryParse(julData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedjul = int.tryParse(julData['med']?.toString() ?? '0') ?? 0;
      int varWaterjul = int.tryParse(julData['water']?.toString() ?? '0') ?? 0;
      int varOther2jul = int.tryParse(julData['other']?.toString() ?? '0') ?? 0;
      int varDdext1jul =
          int.tryParse(julData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2jul =
          int.tryParse(julData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3jul =
          int.tryParse(julData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4jul =
          int.tryParse(julData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedjul =
          int.tryParse(julData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryjul =
          int.tryParse(julData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayaug =
          int.tryParse(augData['splpay']?.toString() ?? '0') ?? 0;
      int varConvaug = int.tryParse(augData['conv']?.toString() ?? '0') ?? 0;
      int varPgaug = int.tryParse(augData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualaug =
          int.tryParse(augData['annual']?.toString() ?? '0') ?? 0;
      int varUniformaug =
          int.tryParse(augData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingaug =
          int.tryParse(augData['nursing']?.toString() ?? '0') ?? 0;
      int varTaaug = int.tryParse(augData['ta']?.toString() ?? '0') ?? 0;
      int varDaontaaug =
          int.tryParse(augData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalaug =
          int.tryParse(augData['medical']?.toString() ?? '0') ?? 0;
      int varDirtaug = int.tryParse(augData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingaug =
          int.tryParse(augData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((augData['nps'] is int
                  ? augData['nps']
                  : int.tryParse(augData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempaug = (([
      ...
      int varDaext1aug =
          int.tryParse(augData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2aug =
          int.tryParse(augData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3aug =
          int.tryParse(augData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4aug =
          int.tryParse(augData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryaug = [
        varBpaug,
        varDaaug,
        varHraaug,
      ...
      int varIncometaxaug =
          int.tryParse(augData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyaug =
          int.tryParse(augData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryaug =
          int.tryParse(augData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varMedaug = int.tryParse(augData['med']?.toString() ?? '0') ?? 0;
      int varWateraug = int.tryParse(augData['water']?.toString() ?? '0') ?? 0;
      int varOther2aug = int.tryParse(augData['other']?.toString() ?? '0') ?? 0;
      int varDdext1aug =
          int.tryParse(augData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2aug =
          int.tryParse(augData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3aug =
          int.tryParse(augData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4aug =
          int.tryParse(augData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedaug =
          int.tryParse(augData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryaug =
          int.tryParse(augData['netsalary']?.toString() ?? '0') ?? 0;

      // ============================= arrearqtr2 ==============================
      int varBparrearqtr2 = int.tryParse('0') ?? 0;
      int varDaarrearqtr2 =
          int.tryParse(arrearData['jada']?.toString() ?? '0') ?? 0;
      int varHraarrearqtr2 =
          int.tryParse(arrearData['jahra']?.toString() ?? '0') ?? 0;
      int varNpaarrearqtr2 =
          int.tryParse(arrearData['janpa']?.toString() ?? '0') ?? 0;
      ...
      int varDaontaarrearqtr2 =
          int.tryParse(arrearData['jadaonta']?.toString() ?? '0') ?? 0;
      ...
      int varPcaarrearqtr2 =
          int.tryParse(arrearData['japca']?.toString() ?? '0') ?? 0;
      int varNpsemparrearqtr2 = int.tryParse('0') ?? 0;
      int varOtherarrearqtr2 =
          int.tryParse(arrearData['jaother']?.toString() ?? '0') ?? 0;
      ...
      int varSalaryarrearqtr2 = [
        ...
        varPgarrearqtr2,
        varAnnualarrearqtr2,
        varUniformarrearqtr2,
        varNursingarrearqtr2,
        varTaarrearqtr2,
        varDaontaarrearqtr2,
        varMedicalarrearqtr2,
        varDirtarrearqtr2,
        varWashingarrearqtr2,
        varTbarrearqtr2,
        varNightarrearqtr2,
        varDrivearrearqtr2,
        varCyclearrearqtr2,
      ...
      int varIncometaxarrearqtr2 =
          int.tryParse(arrearData['jaext1']?.toString() ?? '0') ?? 0;
      ...
      int varTotaldedarrearqtr2 = [
        varIncometaxarrearqtr2,
        varGisarrearqtr2,
        varGpfarrearqtr2,
        varNpsarrearqtr2,
        varSlfarrearqtr2,
        varSocietyarrearqtr2,
        varRecoveryarrearqtr2,
        varWfarrearqtr2,
        varEpfarrearqtr2,
        varEsiarrearqtr2,
        varMedarrearqtr2,
      ...
      int varNetsalaryarrearqtr2 = varSalaryarrearqtr2 - varTotaldedarrearqtr2;

      // =============================== qtr2 ===============================
      int varBpqtr2 = [varBpjun, varBpjul, varBpaug, varBparrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varDaqtr2 = [varDajun, varDajul, varDaaug, varDaarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varHraqtr2 = [varHrajun, varHrajul, varHraaug, varHraarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varNpaqtr2 = [varNpajun, varNpajul, varNpaaug, varNpaarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varSplpayqtr2 = [
        varSplpayjun,
        varSplpayjul,
      ...
      int varConvqtr2 = [varConvjun, varConvjul, varConvaug, varConvarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varPgqtr2 = [varPgjun, varPgjul, varPgaug, varPgarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varAnnualqtr2 = [
        varAnnualjun,
        varAnnualjul,
      ...
      int varTaqtr2 = [varTajun, varTajul, varTaaug, varTaarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varDaontaqtr2 = [
        varDaontajun,
        varDaontajul,
      ...
      int varDirtqtr2 = [varDirtjun, varDirtjul, varDirtaug, varDirtarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varWashingqtr2 = [
        varWashingjun,
        varWashingjul,
      ...
      int varTbqtr2 = [varTbjun, varTbjul, varTbaug, varTbarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varNightqtr2 = [
        varNightjun,
        varNightjul,
      ...
      int varPcaqtr2 = [varPcajun, varPcajul, varPcaaug, varPcaarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varNpsempqtr2 = [
        varNpsempjun,
        varNpsempjul,
      ...
      int varIncometaxqtr2 = [
        varIncometaxjun,
        varIncometaxjul,
        varIncometaxaug,
        varIncometaxarrearqtr2
      ].fold(0, (sum, value) => sum + value);
      int varGisqtr2 = [varGisjun, varGisjul, varGisaug, varGisarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varGpfqtr2 = [varGpfjun, varGpfjul, varGpfaug, varGpfarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varNpsqtr2 = [varNpsjun, varNpsjul, varNpsaug, varNpsarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varSlfqtr2 = [varSlfjun, varSlfjul, varSlfaug, varSlfarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varSocietyqtr2 = [
        varSocietyjun,
        varSocietyjul,
      ...
      int varWfqtr2 = [varWfjun, varWfjul, varWfaug, varWfarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varEpfqtr2 = [varEpfjun, varEpfjul, varEpfaug, varEpfarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varEsiqtr2 = [varEsijun, varEsijul, varEsiaug, varEsiarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varMedqtr2 = [varMedjun, varMedjul, varMedaug, varMedarrearqtr2]
          .fold(0, (sum, value) => sum + value);
      int varWaterqtr2 = [
        varWaterjun,
        varWaterjul,
      ...
      int varTotaldedqtr2 = [
        ...
        varTotaldedjul,
        varTotaldedaug,
        varTotaldedarrearqtr2
      ].fold(0, (sum, value) => sum + value);
      int varNetsalaryqtr2 = [
        varNetsalaryjun,
        varNetsalaryjul,
        varNetsalaryaug,
        varNetsalaryarrearqtr2
      ].fold(0, (sum, value) => sum + value);
      ...
      int varSplpaysept =
          int.tryParse(septData['splpay']?.toString() ?? '0') ?? 0;
      int varConvsept = int.tryParse(septData['conv']?.toString() ?? '0') ?? 0;
      int varPgsept = int.tryParse(septData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualsept =
          int.tryParse(septData['annual']?.toString() ?? '0') ?? 0;
      int varUniformsept =
          int.tryParse(septData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingsept =
          int.tryParse(septData['nursing']?.toString() ?? '0') ?? 0;
      int varTasept = int.tryParse(septData['ta']?.toString() ?? '0') ?? 0;
      int varDaontasept =
          int.tryParse(septData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalsept =
          int.tryParse(septData['medical']?.toString() ?? '0') ?? 0;
      int varDirtsept = int.tryParse(septData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingsept =
          int.tryParse(septData['washing']?.toString() ?? '0') ?? 0;
      int varTbsept = int.tryParse(septData['tb']?.toString() ?? '0') ?? 0;
      int varNightsept =
          int.tryParse(septData['night']?.toString() ?? '0') ?? 0;
      int varDrivesept =
          int.tryParse(septData['drive']?.toString() ?? '0') ?? 0;
      int varCyclesept =
          int.tryParse(septData['cycle']?.toString() ?? '0') ?? 0;
      int varPcasept = int.tryParse(septData['pca']?.toString() ?? '0') ?? 0;
      int varNpsempsept = 0;

      if ((septData['nps'] is int
                  ? septData['nps']
                  : int.tryParse(septData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempsept = (([
      ...
      int varDaext1sept =
          int.tryParse(septData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2sept =
          int.tryParse(septData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3sept =
          int.tryParse(septData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4sept =
          int.tryParse(septData['daext4']?.toString() ?? '0') ?? 0;
      int varSalarysept = [
        varBpsept,
        varDasept,
        varHrasept,
      ...
      int varIncometaxsept =
          int.tryParse(septData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietysept =
          int.tryParse(septData['society']?.toString() ?? '0') ?? 0;
      int varRecoverysept =
          int.tryParse(septData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varWatersept =
          int.tryParse(septData['water']?.toString() ?? '0') ?? 0;
      int varOther2sept =
          int.tryParse(septData['other']?.toString() ?? '0') ?? 0;
      int varMedsept = int.tryParse(septData['med']?.toString() ?? '0') ?? 0;
      int varDdext1sept =
          int.tryParse(septData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2sept =
          int.tryParse(septData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3sept =
          int.tryParse(septData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4sept =
          int.tryParse(septData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedsept =
          int.tryParse(septData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalarysept =
          int.tryParse(septData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayoct =
          int.tryParse(octData['splpay']?.toString() ?? '0') ?? 0;
      int varConvoct = int.tryParse(octData['conv']?.toString() ?? '0') ?? 0;
      int varPgoct = int.tryParse(octData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualoct =
          int.tryParse(octData['annual']?.toString() ?? '0') ?? 0;
      int varUniformoct =
          int.tryParse(octData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingoct =
          int.tryParse(octData['nursing']?.toString() ?? '0') ?? 0;
      int varTaoct = int.tryParse(octData['ta']?.toString() ?? '0') ?? 0;
      int varDaontaoct =
          int.tryParse(octData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicaloct =
          int.tryParse(octData['medical']?.toString() ?? '0') ?? 0;
      int varDirtoct = int.tryParse(octData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingoct =
          int.tryParse(octData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((octData['nps'] is int
                  ? octData['nps']
                  : int.tryParse(octData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempoct = (([
      ...
      int varDaext1oct =
          int.tryParse(octData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2oct =
          int.tryParse(octData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3oct =
          int.tryParse(octData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4oct =
          int.tryParse(octData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryoct = [
        varBpoct,
        varDaoct,
        varHraoct,
      ...
      int varIncometaxoct =
          int.tryParse(octData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyoct =
          int.tryParse(octData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryoct =
          int.tryParse(octData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varDdext1oct =
          int.tryParse(octData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2oct =
          int.tryParse(octData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3oct =
          int.tryParse(octData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4oct =
          int.tryParse(octData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedoct =
          int.tryParse(octData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryoct =
          int.tryParse(octData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpaynov =
          int.tryParse(novData['splpay']?.toString() ?? '0') ?? 0;
      int varConvnov = int.tryParse(novData['conv']?.toString() ?? '0') ?? 0;
      int varPgnov = int.tryParse(novData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualnov =
          int.tryParse(novData['annual']?.toString() ?? '0') ?? 0;
      int varUniformnov =
          int.tryParse(novData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingnov =
          int.tryParse(novData['nursing']?.toString() ?? '0') ?? 0;
      int varTanov = int.tryParse(novData['ta']?.toString() ?? '0') ?? 0;
      int varDaontanov =
          int.tryParse(novData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalnov =
          int.tryParse(novData['medical']?.toString() ?? '0') ?? 0;
      int varDirtnov = int.tryParse(novData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingnov =
          int.tryParse(novData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((novData['nps'] is int
                  ? novData['nps']
                  : int.tryParse(novData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempnov = (([
      ...
      int varDaext1nov =
          int.tryParse(novData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2nov =
          int.tryParse(novData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3nov =
          int.tryParse(novData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4nov =
          int.tryParse(novData['daext4']?.toString() ?? '0') ?? 0;
      int varSalarynov = [
        varBpnov,
        varDanov,
        varHranov,
      ...
      int varIncometaxnov =
          int.tryParse(novData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietynov =
          int.tryParse(novData['society']?.toString() ?? '0') ?? 0;
      int varRecoverynov =
          int.tryParse(novData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varDdext1nov =
          int.tryParse(novData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2nov =
          int.tryParse(novData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3nov =
          int.tryParse(novData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4nov =
          int.tryParse(novData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldednov =
          int.tryParse(novData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalarynov =
          int.tryParse(novData['netsalary']?.toString() ?? '0') ?? 0;

      // ============================= arrearqtr3 ==============================
      int varBparrearqtr3 = int.tryParse('0') ?? 0;
      int varDaarrearqtr3 =
          int.tryParse(arrearData['snda']?.toString() ?? '0') ?? 0;
      int varHraarrearqtr3 =
          int.tryParse(arrearData['snhra']?.toString() ?? '0') ?? 0;
      int varNpaarrearqtr3 =
          int.tryParse(arrearData['snnpa']?.toString() ?? '0') ?? 0;
      ...
      int varDaontaarrearqtr3 =
          int.tryParse(arrearData['sndaonta']?.toString() ?? '0') ?? 0;
      ...
      int varPcaarrearqtr3 =
          int.tryParse(arrearData['snpca']?.toString() ?? '0') ?? 0;
      int varNpsemparrearqtr3 = int.tryParse('0') ?? 0;
      int varOtherarrearqtr3 =
          int.tryParse(arrearData['snother']?.toString() ?? '0') ?? 0;
      ...
      int varSalaryarrearqtr3 = [
        ...
        varPgarrearqtr3,
        varAnnualarrearqtr3,
        varUniformarrearqtr3,
        varNursingarrearqtr3,
        varTaarrearqtr3,
        varDaontaarrearqtr3,
        varMedicalarrearqtr3,
        varDirtarrearqtr3,
        varWashingarrearqtr3,
        varTbarrearqtr3,
        varNightarrearqtr3,
        varDrivearrearqtr3,
        varCyclearrearqtr3,
      ...
      int varIncometaxarrearqtr3 =
          int.tryParse(arrearData['snext1']?.toString() ?? '0') ?? 0;
      ...
      int varTotaldedarrearqtr3 = [
        varIncometaxarrearqtr3,
        varGisarrearqtr3,
        varGpfarrearqtr3,
        varNpsarrearqtr3,
        varSlfarrearqtr3,
        varSocietyarrearqtr3,
        varRecoveryarrearqtr3,
        varWfarrearqtr3,
        varEpfarrearqtr3,
        varEsiarrearqtr3,
        varMedarrearqtr3,
      ...
      int varNetsalaryarrearqtr3 = varSalaryarrearqtr3 - varTotaldedarrearqtr3;

      // =============================== qtr3 ===============================
      int varBpqtr3 = [varBpsept, varBpoct, varBpnov, varBparrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varDaqtr3 = [varDasept, varDaoct, varDanov, varDaarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varHraqtr3 = [varHrasept, varHraoct, varHranov, varHraarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varNpaqtr3 = [varNpasept, varNpaoct, varNpanov, varNpaarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varSplpayqtr3 = [
        varSplpaysept,
        varSplpayoct,
      ...
      int varConvqtr3 = [varConvsept, varConvoct, varConvnov, varConvarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varPgqtr3 = [varPgsept, varPgoct, varPgnov, varPgarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varAnnualqtr3 = [
        varAnnualsept,
        varAnnualoct,
      ...
      int varTaqtr3 = [varTasept, varTaoct, varTanov, varTaarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varDaontaqtr3 = [
        varDaontasept,
        varDaontaoct,
      ...
      int varDirtqtr3 = [varDirtsept, varDirtoct, varDirtnov, varDirtarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varWashingqtr3 = [
        varWashingsept,
        varWashingoct,
      ...
      int varTbqtr3 = [varTbsept, varTboct, varTbnov, varTbarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varNightqtr3 = [
        varNightsept,
        varNightoct,
      ...
      int varPcaqtr3 = [varPcasept, varPcaoct, varPcanov, varPcaarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varNpsempqtr3 = [
        varNpsempsept,
        varNpsempoct,
      ...
      int varIncometaxqtr3 = [
        varIncometaxsept,
        varIncometaxoct,
        varIncometaxnov,
        varIncometaxarrearqtr3
      ].fold(0, (sum, value) => sum + value);
      int varGisqtr3 = [varGissept, varGisoct, varGisnov, varGisarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varGpfqtr3 = [varGpfsept, varGpfoct, varGpfnov, varGpfarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varNpsqtr3 = [varNpssept, varNpsoct, varNpsnov, varNpsarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varSlfqtr3 = [varSlfsept, varSlfoct, varSlfnov, varSlfarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varSocietyqtr3 = [
        varSocietysept,
        varSocietyoct,
      ...
      int varWfqtr3 = [varWfsept, varWfoct, varWfnov, varWfarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varEpfqtr3 = [varEpfsept, varEpfoct, varEpfnov, varEpfarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varEsiqtr3 = [varEsisept, varEsioct, varEsinov, varEsiarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varMedqtr3 = [varMedsept, varMedoct, varMednov, varMedarrearqtr3]
          .fold(0, (sum, value) => sum + value);
      int varWaterqtr3 = [
        varWatersept,
        varWateroct,
      ...
      int varTotaldedqtr3 = [
        ...
        varTotaldedoct,
        varTotaldednov,
        varTotaldedarrearqtr3
      ].fold(0, (sum, value) => sum + value);
      int varNetsalaryqtr3 = [
        varNetsalarysept,
        varNetsalaryoct,
        varNetsalarynov,
        varNetsalaryarrearqtr3
      ].fold(0, (sum, value) => sum + value);
      ...
      int varSplpaydec =
          int.tryParse(decData['splpay']?.toString() ?? '0') ?? 0;
      int varConvdec = int.tryParse(decData['conv']?.toString() ?? '0') ?? 0;
      int varPgdec = int.tryParse(decData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualdec =
          int.tryParse(decData['annual']?.toString() ?? '0') ?? 0;
      int varUniformdec =
          int.tryParse(decData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingdec =
          int.tryParse(decData['nursing']?.toString() ?? '0') ?? 0;
      int varTadec = int.tryParse(decData['ta']?.toString() ?? '0') ?? 0;
      int varDaontadec =
          int.tryParse(decData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicaldec =
          int.tryParse(decData['medical']?.toString() ?? '0') ?? 0;
      int varDirtdec = int.tryParse(decData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingdec =
          int.tryParse(decData['washing']?.toString() ?? '0') ?? 0;
      ...
      int varNpsempdec = 0;
      if ((decData['nps'] is int
                  ? decData['nps']
                  : int.tryParse(decData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempdec = (([
      ...
      int varDaext1dec =
          int.tryParse(decData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2dec =
          int.tryParse(decData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3dec =
          int.tryParse(decData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4dec =
          int.tryParse(decData['daext4']?.toString() ?? '0') ?? 0;
      int varSalarydec = [
        varBpdec,
        varDadec,
        varHradec,
      ...
      int varIncometaxdec =
          int.tryParse(decData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietydec =
          int.tryParse(decData['society']?.toString() ?? '0') ?? 0;
      int varRecoverydec =
          int.tryParse(decData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varDdext1dec =
          int.tryParse(decData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2dec =
          int.tryParse(decData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3dec =
          int.tryParse(decData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4dec =
          int.tryParse(decData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldeddec =
          int.tryParse(decData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalarydec =
          int.tryParse(decData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayjan =
          int.tryParse(janData['splpay']?.toString() ?? '0') ?? 0;
      int varConvjan = int.tryParse(janData['conv']?.toString() ?? '0') ?? 0;
      int varPgjan = int.tryParse(janData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualjan =
          int.tryParse(janData['annual']?.toString() ?? '0') ?? 0;
      int varUniformjan =
          int.tryParse(janData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingjan =
          int.tryParse(janData['nursing']?.toString() ?? '0') ?? 0;
      int varTajan = int.tryParse(janData['ta']?.toString() ?? '0') ?? 0;
      int varDaontajan =
          int.tryParse(janData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicaljan =
          int.tryParse(janData['medical']?.toString() ?? '0') ?? 0;
      int varDirtjan = int.tryParse(janData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingjan =
          int.tryParse(janData['washing']?.toString() ?? '0') ?? 0;
      ...
      if ((janData['nps'] is int
                  ? janData['nps']
                  : int.tryParse(janData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempjan = (([
      ...
      int varDaext1jan =
          int.tryParse(janData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2jan =
          int.tryParse(janData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3jan =
          int.tryParse(janData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4jan =
          int.tryParse(janData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryjan = [
        varBpjan,
        varDajan,
        varHrajan,
      ...
      int varIncometaxjan =
          int.tryParse(janData['incometax']?.toString() ?? '0') ?? 0;
      ...
      int varSocietyjan =
          int.tryParse(janData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryjan =
          int.tryParse(janData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varDdext1jan =
          int.tryParse(janData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2jan =
          int.tryParse(janData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3jan =
          int.tryParse(janData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4jan =
          int.tryParse(janData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedjan =
          int.tryParse(janData['totalded']?.toString() ?? '0') ?? 0;
      int varNetsalaryjan =
          int.tryParse(janData['netsalary']?.toString() ?? '0') ?? 0;
      ...
      int varSplpayfeb =
          int.tryParse(febData['splpay']?.toString() ?? '0') ?? 0;
      int varConvfeb = int.tryParse(febData['conv']?.toString() ?? '0') ?? 0;
      int varPgfeb = int.tryParse(febData['pg']?.toString() ?? '0') ?? 0;
      int varAnnualfeb =
          int.tryParse(febData['annual']?.toString() ?? '0') ?? 0;
      int varUniformfeb =
          int.tryParse(febData['uniform']?.toString() ?? '0') ?? 0;
      int varNursingfeb =
          int.tryParse(febData['nursing']?.toString() ?? '0') ?? 0;
      int varTafeb = int.tryParse(febData['ta']?.toString() ?? '0') ?? 0;
      int varDaontafeb =
          int.tryParse(febData['daonta']?.toString() ?? '0') ?? 0;
      int varMedicalfeb =
          int.tryParse(febData['medical']?.toString() ?? '0') ?? 0;
      int varDirtfeb = int.tryParse(febData['dirt']?.toString() ?? '0') ?? 0;
      int varWashingfeb =
          int.tryParse(febData['washing']?.toString() ?? '0') ?? 0;
      ...
      int varNpsempfeb = 0;
      if ((febData['nps'] is int
                  ? febData['nps']
                  : int.tryParse(febData['nps']?.toString() ?? '0') ?? 0) >
              0 &&
          !isECR) {
        varNpsempfeb = (([
      ...
      int varDaext1feb =
          int.tryParse(febData['daext1']?.toString() ?? '0') ?? 0;
      int varDaext2feb =
          int.tryParse(febData['daext2']?.toString() ?? '0') ?? 0;
      int varDaext3feb =
          int.tryParse(febData['daext3']?.toString() ?? '0') ?? 0;
      int varDaext4feb =
          int.tryParse(febData['daext4']?.toString() ?? '0') ?? 0;
      int varSalaryfeb = [
        varBpfeb,
        varDafeb,
        varHrafeb,
      ...
      int varIncometaxfeb = !isECR
          ? int.tryParse(febData['incometax']?.toString() ?? '0') ?? 0
          : globalNitpi;
      ...
      int varSocietyfeb =
          int.tryParse(febData['society']?.toString() ?? '0') ?? 0;
      int varRecoveryfeb =
          int.tryParse(febData['recovery']?.toString() ?? '0') ?? 0;
      ...
      int varDdext1feb =
          int.tryParse(febData['ddext1']?.toString() ?? '0') ?? 0;
      int varDdext2feb =
          int.tryParse(febData['ddext2']?.toString() ?? '0') ?? 0;
      int varDdext3feb =
          int.tryParse(febData['ddext3']?.toString() ?? '0') ?? 0;
      int varDdext4feb =
          int.tryParse(febData['ddext4']?.toString() ?? '0') ?? 0;
      int varTotaldedfeb =
          int.tryParse(febData['totalded']?.toString() ?? '0') ?? 0;
      varTotaldedfeb =
          isECR ? varTotaldedfeb + varIncometaxfeb : varTotaldedfeb;
      int varNetsalaryfeb =
          int.tryParse(febData['netsalary']?.toString() ?? '0') ?? 0;
      varNetsalaryfeb =
          isECR ? varNetsalaryfeb - varIncometaxfeb : varNetsalaryfeb;

      // ============================= arrearqtr4 ==============================
      int varBparrearqtr4 = int.tryParse('0') ?? 0;
      int varDaarrearqtr4 =
          int.tryParse(arrearData['dfda']?.toString() ?? '0') ?? 0;
      int varHraarrearqtr4 =
          int.tryParse(arrearData['dfhra']?.toString() ?? '0') ?? 0;
      int varNpaarrearqtr4 =
          int.tryParse(arrearData['dfnpa']?.toString() ?? '0') ?? 0;
      ...
      int varDaontaarrearqtr4 =
          int.tryParse(arrearData['dfdaonta']?.toString() ?? '0') ?? 0;
      ...
      int varPcaarrearqtr4 =
          int.tryParse(arrearData['dfpca']?.toString() ?? '0') ?? 0;
      int varNpsemparrearqtr4 = int.tryParse('0') ?? 0;
      int varOtherarrearqtr4 =
          int.tryParse(arrearData['dfother']?.toString() ?? '0') ?? 0;
      ...
      int varSalaryarrearqtr4 = [
        ...
        varPgarrearqtr4,
        varAnnualarrearqtr4,
        varUniformarrearqtr4,
        varNursingarrearqtr4,
        varTaarrearqtr4,
        varDaontaarrearqtr4,
        varMedicalarrearqtr4,
        varDirtarrearqtr4,
        varWashingarrearqtr4,
        varTbarrearqtr4,
        varNightarrearqtr4,
        varDrivearrearqtr4,
        varCyclearrearqtr4,
      ...
      int varIncometaxarrearqtr4 =
          int.tryParse(arrearData['dfext1']?.toString() ?? '0') ?? 0;
      ...
      int varTotaldedarrearqtr4 = [
        varIncometaxarrearqtr4,
        varGisarrearqtr4,
        varGpfarrearqtr4,
        varNpsarrearqtr4,
        varSlfarrearqtr4,
        varSocietyarrearqtr4,
        varRecoveryarrearqtr4,
        varWfarrearqtr4,
        varEpfarrearqtr4,
        varEsiarrearqtr4,
        varMedarrearqtr4,
      ...
      int varNetsalaryarrearqtr4 = varSalaryarrearqtr4 - varTotaldedarrearqtr4;

      //===========================tution & bonus===========================
      int varTution = [
        int.tryParse(arrearData['mmtution']?.toString() ?? '0') ?? 0,
        int.tryParse(arrearData['jatution']?.toString() ?? '0') ?? 0,
        int.tryParse(arrearData['sntution']?.toString() ?? '0') ?? 0,
        int.tryParse(arrearData['dftution']?.toString() ?? '0') ?? 0
      ].fold(0, (sum, value) => sum + value);
      int varSalarytution = varTution;
      int varBonus = int.tryParse(arrearData['bonus']?.toString() ?? '0') ?? 0;
      int varSalarybonus = varBonus;

      int varTotaldedtution = int.tryParse("0") ?? 0;
      int varTotaldedbonus = int.tryParse("0") ?? 0;
      int varNetsalarytution = varSalarytution - varTotaldedtution;
      int varNetsalarybonus = varSalarybonus - varTotaldedbonus;

      // =============================== qtr4 ===============================
      int varBpqtr4 = [varBpdec, varBpjan, varBpfeb, varBparrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varDaqtr4 = [varDadec, varDajan, varDafeb, varDaarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varHraqtr4 = [varHradec, varHrajan, varHrafeb, varHraarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpaqtr4 = [varNpadec, varNpajan, varNpafeb, varNpaarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSplpayqtr4 = [
        varSplpaydec,
        varSplpayjan,
      ...
      int varConvqtr4 = [varConvdec, varConvjan, varConvfeb, varConvarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varPgqtr4 = [varPgdec, varPgjan, varPgfeb, varPgarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varAnnualqtr4 = [
        varAnnualdec,
        varAnnualjan,
      ...
      int varTaqtr4 = [varTadec, varTajan, varTafeb, varTaarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varDaontaqtr4 = [
        varDaontadec,
        varDaontajan,
      ...
      int varDirtqtr4 = [varDirtdec, varDirtjan, varDirtfeb, varDirtarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varWashingqtr4 = [
        varWashingdec,
        varWashingjan,
      ...
      int varTbqtr4 = [varTbdec, varTbjan, varTbfeb, varTbarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNightqtr4 = [
        varNightdec,
        varNightjan,
      ...
      int varPcaqtr4 = [varPcadec, varPcajan, varPcafeb, varPcaarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpsempqtr4 = [
        varNpsempdec,
        varNpsempjan,
      ...
      int varIncometaxqtr4 = [
        varIncometaxdec,
        varIncometaxjan,
        varIncometaxfeb,
        varIncometaxarrearqtr4
      ].fold(0, (sum, value) => sum + value);
      int varGisqtr4 = [varGisdec, varGisjan, varGisfeb, varGisarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varGpfqtr4 = [varGpfdec, varGpfjan, varGpffeb, varGpfarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpsqtr4 = [varNpsdec, varNpsjan, varNpsfeb, varNpsarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSlfqtr4 = [varSlfdec, varSlfjan, varSlffeb, varSlfarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSocietyqtr4 = [
        varSocietydec,
        varSocietyjan,
      ...
      int varWfqtr4 = [varWfdec, varWfjan, varWffeb, varWfarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varEpfqtr4 = [varEpfdec, varEpfjan, varEpffeb, varEpfarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varEsiqtr4 = [varEsidec, varEsijan, varEsifeb, varEsiarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varMedqtr4 = [varMeddec, varMedjan, varMedfeb, varMedarrearqtr4]
          .fold(0, (sum, value) => sum + value);
      int varWaterqtr4 = [
        varWaterdec,
        varWaterjan,
      ...
      int varTotaldedqtr4 = [
        ...
        varTotaldedjan,
        varTotaldedfeb,
        varTotaldedarrearqtr4
      ].fold(0, (sum, value) => sum + value);
      int varNetsalaryqtr4 = [
        varNetsalarydec,
        varNetsalaryjan,
        varNetsalaryfeb,
        varNetsalaryarrearqtr4
      ].fold(0, (sum, value) => sum + value);

      // ============================================= gross total ==========================================
      int varBpgross = [varBpqtr1, varBpqtr2, varBpqtr3, varBpqtr4]
          .fold(0, (sum, value) => sum + value);
      int varDagross = [varDaqtr1, varDaqtr2, varDaqtr3, varDaqtr4]
          .fold(0, (sum, value) => sum + value);
      int varHragross = [varHraqtr1, varHraqtr2, varHraqtr3, varHraqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpagross = [varNpaqtr1, varNpaqtr2, varNpaqtr3, varNpaqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSplpaygross = [
        varSplpayqtr1,
        varSplpayqtr2,
      ...
      int varConvgross = [varConvqtr1, varConvqtr2, varConvqtr3, varConvqtr4]
          .fold(0, (sum, value) => sum + value);
      int varPggross = [varPgqtr1, varPgqtr2, varPgqtr3, varPgqtr4]
          .fold(0, (sum, value) => sum + value);
      int varAnnualgross = [
        varAnnualqtr1,
        varAnnualqtr2,
      ...
      int varTagross = [varTaqtr1, varTaqtr2, varTaqtr3, varTaqtr4]
          .fold(0, (sum, value) => sum + value);
      int varDaontagross = [
        varDaontaqtr1,
        varDaontaqtr2,
      ...
      int varDirtgross = [varDirtqtr1, varDirtqtr2, varDirtqtr3, varDirtqtr4]
          .fold(0, (sum, value) => sum + value);
      int varWashinggross = [
        varWashingqtr1,
        varWashingqtr2,
      ...
      int varTbgross = [varTbqtr1, varTbqtr2, varTbqtr3, varTbqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNightgross = [
        varNightqtr1,
        varNightqtr2,
      ...
      int varPcagross = [varPcaqtr1, varPcaqtr2, varPcaqtr3, varPcaqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpsempgross = [
        varNpsempqtr1,
        varNpsempqtr2,
      ...
      int varGisgross = [varGisqtr1, varGisqtr2, varGisqtr3, varGisqtr4]
          .fold(0, (sum, value) => sum + value);
      int varGpfgross = [varGpfqtr1, varGpfqtr2, varGpfqtr3, varGpfqtr4]
          .fold(0, (sum, value) => sum + value);
      int varNpsgross = [varNpsqtr1, varNpsqtr2, varNpsqtr3, varNpsqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSlfgross = [varSlfqtr1, varSlfqtr2, varSlfqtr3, varSlfqtr4]
          .fold(0, (sum, value) => sum + value);
      int varSocietygross = [
        varSocietyqtr1,
        varSocietyqtr2,
      ...
      int varWfgross = [varWfqtr1, varWfqtr2, varWfqtr3, varWfqtr4]
          .fold(0, (sum, value) => sum + value);
      int varEpfgross = [varEpfqtr1, varEpfqtr2, varEpfqtr3, varEpfqtr4]
          .fold(0, (sum, value) => sum + value);
      int varEsigross = [varEsiqtr1, varEsiqtr2, varEsiqtr3, varEsiqtr4]
          .fold(0, (sum, value) => sum + value);
      int varMedgross = [varMedqtr1, varMedqtr2, varMedqtr3, varMedqtr4]
          .fold(0, (sum, value) => sum + value);
      int varWatergross = [
        varWaterqtr1,
        varWaterqtr2,
      ...
      if (varBpgross > 0) {
        ...
        sheet.getRangeByName("${columns[end]}11:${columns[end]}14").cellStyle =
            commontextStyle;
        sheet.getRangeByName("${columns[end]}16:${columns[end]}19").cellStyle =
            commontextStyle;
        sheet.getRangeByName("${columns[end]}21:${columns[end]}24").cellStyle =
            commontextStyle;
        sheet.getRangeByName("${columns[end]}26:${columns[end]}31").cellStyle =
            commontextStyle;

        List<String> qtrcolumns = ['15', '20', '25', '32'];
        for (String cols in qtrcolumns) {
          sheet.getRangeByName("${columns[end]}$cols").cellStyle =
              totalrowStyle!;
        }
      ...
      if (varUniformgross > 0) {
        ...
        sheet
            .getRangeByName("${columns[end]}14")
            .setValue(varUniformarrearqtr1);
        ...
        sheet
            .getRangeByName("${columns[end]}19")
            .setValue(varUniformarrearqtr2);
        ...
        sheet
            .getRangeByName("${columns[end]}24")
            .setValue(varUniformarrearqtr3);
        ...
        sheet
            .getRangeByName("${columns[end]}29")
            .setValue(varUniformarrearqtr4);
        sheet.getRangeByName("${columns[end]}30").setValue('0');
        sheet.getRangeByName("${columns[end]}31").setValue('0');
      ...
      if (!isECR) {
        if (varNpsempgross > 0) {
          ...
          sheet.getRangeByName("${columns[end]}10").cellStyle =
              tableheadingStyle;
          ...
          sheet
              .getRangeByName("${columns[end]}11:${columns[end]}14")
              .cellStyle = commontextStyle;
          sheet
              .getRangeByName("${columns[end]}16:${columns[end]}19")
              .cellStyle = commontextStyle;
          sheet
              .getRangeByName("${columns[end]}21:${columns[end]}24")
              .cellStyle = commontextStyle;
          sheet
              .getRangeByName("${columns[end]}26:${columns[end]}31")
              .cellStyle = commontextStyle;
          ...
          sheet.getRangeByName("${columns[end]}33").cellStyle =
              formlastrowStyle;
      ...
      if (end != 0) {
        if (end > 2) {
          ...
          sheet
              .getRangeByName("${columns[0]}9")
              .setValue("DETAILS OF ALLOWANCES");
          sheet
              .getRangeByName("${columns[0]}9:${columns[end - 1]}9")
              .cellStyle = commontextStyleBold;
          sheet
              .getRangeByName("${columns[0]}9:${columns[end - 1]}9")
              .cellStyle
      ...
      if (varIncometaxgross > 0) {
        ...
        sheet
            .getRangeByName("${columns[end]}14")
            .setValue(varIncometaxarrearqtr1);
        ...
        sheet
            .getRangeByName("${columns[end]}19")
            .setValue(varIncometaxarrearqtr2);
        ...
        sheet
            .getRangeByName("${columns[end]}24")
            .setValue(varIncometaxarrearqtr3);
        ...
        sheet
            .getRangeByName("${columns[end]}29")
            .setValue(varIncometaxarrearqtr4);
        sheet.getRangeByName("${columns[end]}30").setValue('0');
        sheet.getRangeByName("${columns[end]}31").setValue('0');
      ...
      if (varRecoverygross > 0) {
        ...
        sheet
            .getRangeByName("${columns[end]}14")
            .setValue(varRecoveryarrearqtr1);
        ...
        sheet
            .getRangeByName("${columns[end]}19")
            .setValue(varRecoveryarrearqtr2);
        ...
        sheet
            .getRangeByName("${columns[end]}24")
            .setValue(varRecoveryarrearqtr3);
        ...
        sheet
            .getRangeByName("${columns[end]}29")
            .setValue(varRecoveryarrearqtr4);
        sheet.getRangeByName("${columns[end]}30").setValue('0');
        sheet.getRangeByName("${columns[end]}31").setValue('0');
      ...
      if (varMedgross > 0 && isECR) {
        ...
        sheet.getRangeByName("${columns[end]}12").setValue(varMedapr);
        sheet.getRangeByName("${columns[end]}13").setValue(varMedmay);
        sheet.getRangeByName("${columns[end]}14").setValue(varMedarrearqtr1);
        sheet.getRangeByName("${columns[end]}15").setValue(varMedqtr1);
        sheet.getRangeByName("${columns[end]}16").setValue(varMedjun);
        sheet.getRangeByName("${columns[end]}17").setValue(varMedjul);
        sheet.getRangeByName("${columns[end]}18").setValue(varMedaug);
        sheet.getRangeByName("${columns[end]}19").setValue(varMedarrearqtr2);
        sheet.getRangeByName("${columns[end]}20").setValue(varMedqtr2);
        sheet.getRangeByName("${columns[end]}21").setValue(varMedsept);
        sheet.getRangeByName("${columns[end]}22").setValue(varMedoct);
        sheet.getRangeByName("${columns[end]}23").setValue(varMednov);
        sheet.getRangeByName("${columns[end]}24").setValue(varMedarrearqtr3);
        sheet.getRangeByName("${columns[end]}25").setValue(varMedqtr3);
        sheet.getRangeByName("${columns[end]}26").setValue(varMeddec);
        sheet.getRangeByName("${columns[end]}27").setValue(varMedjan);
        sheet.getRangeByName("${columns[end]}28").setValue(varMedfeb);
        sheet.getRangeByName("${columns[end]}29").setValue(varMedarrearqtr4);
        sheet.getRangeByName("${columns[end]}30").setValue('0');
        sheet.getRangeByName("${columns[end]}31").setValue('0');
      ...
      if (isECR) {
        if (varTotaldedgross > 0) {
          ...
          sheet
              .getRangeByName("${columns[end]}14")
              .setValue(varTotaldedarrearqtr1);
          ...
          sheet
              .getRangeByName("${columns[end]}19")
              .setValue(varTotaldedarrearqtr2);
          ...
          sheet
              .getRangeByName("${columns[end]}24")
              .setValue(varTotaldedarrearqtr3);
          ...
          sheet
              .getRangeByName("${columns[end]}29")
              .setValue(varTotaldedarrearqtr4);
          sheet.getRangeByName("${columns[end]}30").setValue(varTotaldedtution);
          sheet.getRangeByName("${columns[end]}31").setValue(varTotaldedbonus);
        ...
        if (varNetsalarygross > 0) {
          ...
          sheet
              .getRangeByName("${columns[end]}14")
              .setValue(varNetsalaryarrearqtr1);
          ...
          sheet
              .getRangeByName("${columns[end]}19")
              .setValue(varNetsalaryarrearqtr2);
          ...
          sheet
              .getRangeByName("${columns[end]}24")
              .setValue(varNetsalaryarrearqtr3);
          ...
          sheet
              .getRangeByName("${columns[end]}29")
              .setValue(varNetsalaryarrearqtr4);
          sheet
              .getRangeByName("${columns[end]}30")
              .setValue(varNetsalarytution);
          sheet.getRangeByName("${columns[end]}31").setValue(varNetsalarybonus);
      ...
      if (dstart != end) {
        if ((dstart - end) > 2) {
          sheet
              .getRangeByName("${columns[dstart]}9:${columns[end - 1]}9")
              .merge();
          sheet
              .getRangeByName("${columns[dstart]}9")
              .setValue("DETAILS OF DEDUCTIONS");
          sheet
              .getRangeByName("${columns[dstart]}9:${columns[end - 1]}9")
              .cellStyle = commontextStyleBold;
          sheet
      ...
      if (!isECR) {
        ...
        sheet.getRangeByName('A35:C35').cellStyle.borders.all.lineStyle =
            xls.LineStyle.none;
        sheet
            .getRangeByName('A35')
            .setValue('I have Opted NEW / OLD TAX REGIME');

        sheet.getRangeByName('A39:B39').merge();
        sheet.getRangeByName('A39:B39').cellStyle = commontextStyleBold;
        sheet.getRangeByName('A39:B39').cellStyle.borders.all.lineStyle =
            xls.LineStyle.none;
  ...
  Future<void> _computationPage(
    ...
    try {
      ...
      for (int i = 1; i <= 9; i++) {
        ...
        sheet.getRangeByName("B${i + 2}").cellStyle.hAlign =
      ...
      for (int i = 1; i <= 17; i++) {
        ...
        sheet.getRangeByName("B${i + 20}").cellStyle.hAlign =
            xls.HAlignType.center;
      }
      ...
      sheet.getRangeByName("B20:D20").cellStyle.borders.all.lineStyle =
          xls.LineStyle.thick;
      ...
      sheet
          .getRangeByName("C25")
          .setValue("5 YEARS(FIXED DEPOSIT/TIME DEPOSITE SCHEME)");
      ...
      sheet
          .getRangeByName("C37")
          .setValue("TOTAL SAVINGS DOES NOT EXCEED RS.150000/-");
      ...
      sheet.getRangeByName("B39:D39").cellStyle.borders.all.lineStyle =
          xls.LineStyle.thick;
      ...
      sheet
          .getRangeByName("B48")
          .setValue("DEDUCTION FOR HANDICAPPED PERSON U/S 80U");
      sheet.getRangeByName("B49").setValue("80E (EDUCATION LOAN)");
      sheet.getRangeByName("B50").setValue("80EE");
      sheet
          .getRangeByName("B51")
          .setValue("CONVEYANCE AND CONTIGENCY EXEMPTED");
      ...
      for (int i = 3; i <= 18; i++) {
        ...
        sheet.getRangeByName("D$i").cellStyle.borders.right.lineStyle =
            xls.LineStyle.thick;
      }

      sheet
          .getRangeByName("D3")
          .setValue(int.tryParse(itfData['tbp']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D4")
          .setValue(int.tryParse(itfData['tda']?.toString() ?? '0') ?? 0);
      int varHra = int.tryParse(itfData['thra']?.toString() ?? '0') ?? 0;
      sheet.getRangeByName("D5").setValue(varHra);
      sheet.getRangeByName("D6").setValue(
          (int.tryParse(itfData['tnpa']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tsplpay']?.toString() ?? '0') ?? 0));
      sheet
          .getRangeByName("D7")
          .setValue(int.tryParse(itfData['tnursing']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D8")
          .setValue(int.tryParse(itfData['tpca']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D9").setValue(
          (int.tryParse(itfData['tta']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tdaonta']?.toString() ?? '0') ?? 0));
      sheet
          .getRangeByName("D10")
          .setValue(int.tryParse(itfData['tnpsemp']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D11").setValue(
          (int.tryParse(itfData['tother']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tconv']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tdrive']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tuniform']?.toString() ?? '0') ?? 0));

      sheet
          .getRangeByName("D12")
          .setValue(int.tryParse(itfData['tgross']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D12").cellStyle = commontextStyleBold;

      sheet.getRangeByName("D13").setValue(
          (int.tryParse(itfData['tgross']?.toString() ?? '0') ?? 0).round());
      ...
      int varBasic = (((int.tryParse(itfData['tbp']?.toString() ?? '0') ?? 0) +
                  (int.tryParse(itfData['tda']?.toString() ?? '0') ?? 0) +
                  (int.tryParse(itfData['tnpa']?.toString() ?? '0') ?? 0)) *
              0.5)
          .round();
      ...
      sheet
          .getRangeByName("D18")
          .setValue([varBasic, varRpaid, varHra].reduce(min));
      sheet.getRangeByName("D18").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
      ...
      sheet
          .getRangeByName("D21")
          .setValue(int.tryParse(dedData['po']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D21").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thick;

      sheet
          .getRangeByName("D22")
          .setValue(int.tryParse(dedData['ppf']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D23")
          .setValue(int.tryParse(dedData['lic']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D24")
          .setValue(int.tryParse(dedData['hlp']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D25")
          .setValue(int.tryParse(dedData['fd']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D26")
          .setValue(int.tryParse(dedData['nsc']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D27")
          .setValue(int.tryParse(dedData['80c']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D28")
          .setValue(int.tryParse(dedData['80ccd1']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D29")
          .setValue(int.tryParse(dedData['gpf']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D30")
          .setValue(int.tryParse(dedData['gis']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D31")
          .setValue(int.tryParse(dedData['ulip']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D32")
          .setValue(int.tryParse(dedData['elss']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D33")
          .setValue(int.tryParse(dedData['ssy']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D34")
          .setValue(int.tryParse(dedData['tution']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D35")
          .setValue(int.tryParse(dedData['ext3']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D36")
          .setValue(int.tryParse(dedData['totalsav']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D36").cellStyle = commontextStyleBold;
      sheet.getRangeByName("D36").cellStyle.backColor = "#EEECE1";
      sheet
          .getRangeByName("D37")
          .setValue(int.tryParse(dedData['maxsav']?.toString() ?? '0') ?? 0);
      ...
      sheet
          .getRangeByName("D40")
          .setValue(int.tryParse(dedData['80ccd2']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D41")
          .setValue(int.tryParse(dedData['hli']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D42")
          .setValue(int.tryParse(dedData['80g']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D43")
          .setValue(int.tryParse(dedData['cea']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D44")
          .setValue(int.tryParse(dedData['80ccdnps']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D45")
          .setValue(int.tryParse(dedData['80d']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D46")
          .setValue(int.tryParse(dedData['80dp']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D47")
          .setValue(int.tryParse(dedData['80dps']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D48")
          .setValue(int.tryParse(dedData['80u']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D49")
          .setValue(int.tryParse(dedData['80e']?.toString() ?? '0') ?? 0);
      sheet
          .getRangeByName("D50")
          .setValue(int.tryParse(dedData['80ee']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D51").setValue(
          (int.tryParse(itfData['tconv']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tdrive']?.toString() ?? '0') ?? 0));
      sheet
          .getRangeByName("D52")
          .setValue(int.tryParse(dedData['ma']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("D53").setValue(
      ...
      for (int i = 3; i <= 18; i++) {
        ...
        rangeValue.cellStyle.borders.bottom.lineStyle =
            i != 18 ? xls.LineStyle.thin : xls.LineStyle.thick;
        ...
        rangeNum.cellStyle.borders.bottom.lineStyle =
            i != 18 ? xls.LineStyle.thin : xls.LineStyle.thick;
      ...
      for (int i = 21; i <= 37; i++) {
        ...
        rangeValue.cellStyle.borders.left.lineStyle = xls.LineStyle.thin;
        rangeValue.cellStyle.borders.bottom.lineStyle =
            i != 37 ? xls.LineStyle.thin : xls.LineStyle.thick;
        rangeValue.cellStyle.borders.right.lineStyle = xls.LineStyle.thick;
        rangeNum.cellStyle.borders.all.lineStyle = xls.LineStyle.none;
        rangeNum.cellStyle.borders.top.lineStyle = xls.LineStyle.thin;
        rangeNum.cellStyle.borders.right.lineStyle = xls.LineStyle.thin;
      ...
      for (int i = 40; i <= 53; i++) {
        ...
        rangeValue.cellStyle.borders.left.lineStyle = xls.LineStyle.thin;
        rangeValue.cellStyle.borders.bottom.lineStyle =
            i != 53 ? xls.LineStyle.thin : xls.LineStyle.thick;
        rangeValue.cellStyle.borders.right.lineStyle = xls.LineStyle.thick;
        rangeNum.cellStyle.borders.all.lineStyle = xls.LineStyle.none;
        rangeNum.cellStyle.borders.top.lineStyle = xls.LineStyle.thin;
        rangeNum.cellStyle.borders.right.lineStyle = xls.LineStyle.thin;
      ...
      sheet.getRangeByName("C18").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("C18").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C18").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C18").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;

      sheet.getRangeByName("C37").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("C37").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C37").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C37").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C37").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;

      sheet.getRangeByName("C53").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("C53").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C53").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("C53").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
    } catch (error) {
      if (mounted) {
        showDialog(
  ...
  Future<void> _itaxOldPage(
    ...
    try {
      ...
      for (int i = 0; i <= 3; i++) {
        ...
        sheet.getRangeByName("C${i + 9}").cellStyle.hAlign =
            xls.HAlignType.center;
      ...
      for (int i = 0; i <= 1; i++) {
        ...
        sheet.getRangeByName("D${i + 13}").cellStyle.hAlign =
            xls.HAlignType.center;
      }
      ...
      for (int i = 0; i <= 8; i++) {
        ...
        sheet.getRangeByName("D${i + 16}").cellStyle.hAlign =
            xls.HAlignType.center;
      }
      ...
      for (int i = 7; i <= 11; i++) {
        ...
        sheet.getRangeByName("B${i + 19}").cellStyle.hAlign =
            xls.HAlignType.center;
      ...
      for (int i = 0; i <= 5; i++) {
        ...
        sheet.getRangeByName("C${i + 31}").cellStyle.hAlign =
            xls.HAlignType.center;
        sheet.getRangeByName("D${i + 31}:F${i + 31}").merge();
        sheet.getRangeByName("D${i + 31}:F${i + 31}").cellStyle.hAlign =
            xls.HAlignType.left;
      }

      for (int i = 13; i <= 22; i++) {
        sheet.getRangeByName("B${i + 24}").setValue(i);
        sheet.getRangeByName("B${i + 24}").cellStyle.hAlign =
            xls.HAlignType.center;
      ...
      sheet
          .getRangeByName("C4")
          .setValue("Conveyance , Contigency & Uniform Exempted");
      ...
      sheet
          .getRangeByName("D9")
          .setValue("Interest on Housing Loan ( MAX 2,00,000 )");
      ...
      sheet
          .getRangeByName("D31")
          .setValue("Upto Rs. 2,50,000/-                                NIL");
      sheet
          .getRangeByName("D32")
          .setValue("Rs. 2,50,001/- To 5,00,000                     5%");
      sheet
          .getRangeByName("D33")
          .setValue("Rs. 5,00,001/- To Rs. 10,00,000            10%");
      sheet
          .getRangeByName("D34")
          .setValue("EXCEEDING Rs. 15,00,001                       20%");
      ...
      sheet
          .getRangeByName("C38")
          .setValue("Tax rebate (Sec.87A):if applicable");
      ...
      int varConvContUniform = ['tconv', 'tdrive', 'tuniform'].fold(
          0,
          (total, key) =>
              total + (int.tryParse(itfData[key]?.toString() ?? '0') ?? 0));
      sheet.getRangeByName("G4").setValue(varConvContUniform);

      int varBasic = ((['tbp', 'tda', 'tnpa'].fold(
                  0,
                  (total, key) =>
                      total +
                      (int.tryParse(itfData[key]?.toString() ?? '0') ?? 0))) *
              0.5)
          .round();
      ...
      int varAtc = (int.tryParse(dedData['totalsav']?.toString() ?? '0') ?? 0) -
          (int.tryParse(dedData['80ccd1']?.toString() ?? '0') ?? 0);
      ...
      bool seniorCitizen = referenceDate.isAfter(dateDiff) ||
          referenceDate.isAtSameMomentAs(dateDiff);

      if (seniorCitizen) {
        sheet
            .getRangeByName("D31")
            .setValue("Upto Rs. 3,00,000/-                                NIL");
        sheet
            .getRangeByName("D32")
            .setValue("Rs. 3,00,001/- To 5,00,000                     5%");
        if (varTti > 500000) {
          varT2 = 10000;
      ...
      int varDeduct =
          int.tryParse(itfData['tincometax']?.toString() ?? '0') ?? 0;
      ...
      await updateItaxold(biometricId, varNitpi, varEp);
      ...
      for (int i = 3; i <= 46; i++) {
        ...
        xls.Range rangeNum = sheet.getRangeByName("B$i");
        rangeValue.cellStyle = !(excludedNumbers.contains(i))
            ? commontextStyle
            : commontextStyleBold;
      ...
      sheet.getRangeByName("B46:G46").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("B46:G46").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("G46").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("C46:F46").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("B46:G46").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("B46").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("C46:F46").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thin;
      ...
      sheet.getRangeByName("G48").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      ...
      sheet.getRangeByName("B50:G50").cellStyle.borders.all.lineStyle =
          xls.LineStyle.thick;
      ...
      sheet.getRangeByName("E51").setValue(
          "Sub: INCOME TAX FOR THE FINANCIAL YEAR ${sharedData.ccurrentYear}");

      var year =
          (int.tryParse(sharedData.ccurrentYear.substring(0, 4)) ?? 0) + 1;
      ...
      sheet.getRangeByName("G54").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;

      sheet.getRangeByName("B51:G54").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("B51:B54").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("B54:G54").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("G51:G54").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thick;
    } catch (error) {
      if (mounted) {
        showDialog(
  ...
  Future<void> _itaxNewPage(
    ...
    try {
      ...
      sheet.getRangeByName("B12:B21").merge();
      sheet.getRangeByName("B12:B21").cellStyle = commontextStyle!;
      sheet.getRangeByName("B12:B21").cellStyle.hAlign = xls.HAlignType.center;

      for (int i = 11; i <= 20; i++) {
        sheet.getRangeByName("B${i + 11}").setValue(i);
        sheet.getRangeByName("B${i + 11}").cellStyle = commontextStyle;
        sheet.getRangeByName("B${i + 11}").cellStyle.hAlign =
            xls.HAlignType.center;
        sheet.getRangeByName("C${i + 11}:F${i + 11}").merge();
        sheet.getRangeByName("C${i + 11}:F${i + 11}").cellStyle =
            commontextStyle;
      }
      ...
      for (int i = 0; i < romanNumerals.length; i++) {
        ...
        sheet.getRangeByName("C${i + 13}:F${i + 13}").cellStyle =
            commontextStyle;
      }
      if (sharedData.ccurrentYear == '2024-25') {
        ...
        sheet
            .getRangeByName("D14")
            .setValue("Rs. 3,00,001/- To 7,00,000                     5%");
        sheet
            .getRangeByName("D15")
            .setValue("Rs. 7,00,001/- To Rs. 10,00,000            10%");
        sheet
            .getRangeByName("D16")
            .setValue("Rs. 10,00,001/- To Rs. 12,00,000          15%");
        sheet
            .getRangeByName("D17")
            .setValue("Rs. 12,00,001/- To Rs. 15,00,000          20%");
        sheet
            .getRangeByName("D18")
            .setValue("ABOVE Rs. 15,00,001                               30%");
      } else if (sharedData.ccurrentYear == '2025-26' ||
          sharedData.ccurrentYear == '2026-27') {
        sheet
            .getRangeByName("D13")
            .setValue("Upto Rs. 4,00,000/-                                NIL");
        sheet
            .getRangeByName("D14")
            .setValue("Rs. 4,00,001/- To 8,00,000                     5%");
        sheet
            .getRangeByName("D15")
            .setValue("Rs. 8,00,001/- To Rs. 12,00,000            10%");
        sheet
            .getRangeByName("D16")
            .setValue("Rs. 12,00,001/- To Rs. 16,00,000          15%");
        sheet
            .getRangeByName("D17")
            .setValue("Rs. 16,00,001/- To Rs. 20,00,000          20%");
        sheet
            .getRangeByName("D18")
            .setValue("Rs. 20,00,001/- To Rs. 24,00,000          25%");
        sheet
            .getRangeByName("D19")
            .setValue("ABOVE Rs. 24,00,001                               30%");
      }
      sheet.getRangeByName("D20").setValue("TOTAL TAX");
      sheet.getRangeByName("D20").cellStyle = commontextStyleBold;
      sheet.getRangeByName("D21").setValue("SURCHARGE");

      sheet.getRangeByName("C22").setValue("Tax Liability");
      sheet.getRangeByName("C22").cellStyle = commontextStyleBold;
      sheet
          .getRangeByName("C23")
          .setValue("Tax rebate (Sec.87A):if applicable");
      sheet.getRangeByName("C24").setValue("Total Tax Liability");
      sheet.getRangeByName("C24").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C24").rowHeight = 17.40;
      sheet.getRangeByName("C25").setValue("Education Cess  4%");
      sheet.getRangeByName("C25").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C25").rowHeight = 17.40;
      sheet.getRangeByName("C26").setValue("Total Tax Payble Rs.");
      sheet.getRangeByName("C26").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C26").rowHeight = 17.40;
      sheet.getRangeByName("C27").setValue("Deduct :-Income Tax already Paid");
      sheet.getRangeByName("C28").setValue("Income Tax Payble");
      sheet.getRangeByName("C28").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C29").setValue("Relief u/s 89(i)");
      sheet.getRangeByName("C30").rowHeight = 19.80;
      sheet.getRangeByName("C30").setValue("Net Income Tax Payble");
      sheet.getRangeByName("C30").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C30").cellStyle.fontSize = 14;
      sheet.getRangeByName("C30").cellStyle.backColor = "#FBD4B4";
      sheet.getRangeByName("C31").setValue("Excess Paid");
      sheet.getRangeByName("C31").cellStyle = commontextStyleBold;
      sheet.getRangeByName("C31").rowHeight = 17.40;

      for (int i = 3; i <= 31; i++) {
        sheet.getRangeByName("G$i").cellStyle = commontextStyle;
        sheet.getRangeByName("G$i").cellStyle.backColor = "#EEECE1";
        sheet.getRangeByName("G$i").cellStyle.borders.right.lineStyle =
            xls.LineStyle.thick;
      }
      ...
      int varConvDrive =
          (int.tryParse(itfData['tconv']?.toString() ?? '0') ?? 0) +
              (int.tryParse(itfData['tdrive']?.toString() ?? '0') ?? 0);
      sheet.getRangeByName("G4").setValue(varConvDrive);

      int varUniform =
          int.tryParse(itfData['tuniform']?.toString() ?? '0') ?? 0;
      sheet.getRangeByName("G5").setValue(varUniform);
      ...
      if (sharedData.ccurrentYear == '2024-25') {
        ...
        if (varTti > 800000) {
          ...
          varT2 = ((varTti - 400000) * 0.05).round();
        }
        if (varTti > 1200000) {
          varT3 = 40000;
        } else if (varTti > 800000 && varTti <= 1200000) {
          varT3 = ((varTti - 800000) * 0.1).round();
        }
        if (varTti > 1600000) {
          varT4 = 60000;
        } else if (varTti > 1200000 && varTti <= 1600000) {
          int excess = ((varTti - 1200000) * 0.15).round() +
              varT2 +
              varT3 -
              (varTti - 1200000);
          if (excess > 0) {
            if (varTti > 1200000 && varTti < 1275000) {
              varT2 = 0;
              varT3 = 0;
              varT4 = varTti - 1200000;
            }
          } else {
            varT4 = ((varTti - 1200000) * 0.15).round();
          }
        }
        if (varTti > 2000000) {
          varT5 = 80000;
      ...
      sheet.getRangeByName("G20").setValue(varTt);
      sheet.getRangeByName("G21").setValue(varSur);
      ...
      int varDeduct =
          int.tryParse(itfData['tincometax']?.toString() ?? '0') ?? 0;
      sheet.getRangeByName("G27").setValue(varDeduct);

      int varNitp = varTtp - varDeduct;
      if (varNitp < 0) {
        varNitp = 0;
      }
      sheet.getRangeByName("G28").setValue(varNitp);
      ...
      sheet.getRangeByName("G31").setValue(varEp);
      ...
      sheet.getRangeByName("B31:G31").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("B31:G31").cellStyle.borders.top.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("G31").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("C31:F31").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("B31:G31").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("B31").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("C31:F31").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thin;
      sheet.getRangeByName("G30").cellStyle.fontSize = 14;

      sheet.getRangeByName("G33").setValue("A.O/A.A.O/D.D.O");
      sheet.getRangeByName("G33").cellStyle = commontextStyleBold;
      sheet.getRangeByName("G33").cellStyle.hAlign = xls.HAlignType.right;
      sheet.getRangeByName("G33").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;

      sheet.getRangeByName("B35:G35").merge();
      sheet.getRangeByName("B35:G35").setValue(sharedData.zone.toUpperCase());
      sheet.getRangeByName("B35:G35").cellStyle = commontextStyleBold;
      sheet.getRangeByName("B35:G35").cellStyle.borders.all.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("B35:G35").cellStyle.hAlign = xls.HAlignType.center;

      sheet.getRangeByName("B36:D36").merge();
      sheet.getRangeByName("B36:D36").cellStyle = commontextStyle;
      sheet.getRangeByName("B36").setValue(mainpgData["panno"]);

      sheet.getRangeByName("E36:G36").merge();
      sheet.getRangeByName("E36:G36").cellStyle = commontextStyle;
      sheet.getRangeByName("E36").setValue(
          "Sub: INCOME TAX FOR THE FINANCIAL YEAR ${sharedData.ccurrentYear}");
      ...
      sheet.getRangeByName("B40").setValue("Surcharge:");
      sheet.getRangeByName("E40").setFormula('=G21');
      ...
      sheet.getRangeByName("G44").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;

      sheet.getRangeByName("B36:G47").cellStyle.borders.all.lineStyle =
          xls.LineStyle.none;
      sheet.getRangeByName("B36:B47").cellStyle.borders.left.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("B47:G47").cellStyle.borders.bottom.lineStyle =
          xls.LineStyle.thick;
      sheet.getRangeByName("G36:G47").cellStyle.borders.right.lineStyle =
          xls.LineStyle.thick;
    } catch (error) {
      if (mounted) {
        showDialog(
  ...
  Future<void> exportAll([bool isPan = false]) async {
    try {
      ...
      for (int i = startIndex; i < biometricIds.length; i++) {
        await createExcel(
            biometricId: biometricIds[i], isPan: isPan, isExportAll: true);
      }
    } catch (error) {
      if (mounted) {
        showDialog(
